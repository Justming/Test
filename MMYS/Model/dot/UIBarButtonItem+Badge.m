...
- (void)badgeInit
{
    ...
    if (self.customView) {
        ...
        defaultOriginX = superview.frame.size.width/2 - self.badge.frame.size.width/2;
        // Avoids badge to be clipped when animating its scale
        superview.clipsToBounds = NO;
    } else if ([self respondsToSelector:@selector(view)] && [(id)self view]) {
        superview = [(id)self view];
        defaultOriginX = superview.frame.size.width - self.badge.frame.size.width/26;
    }
    [superview addSubview:self.badge];
    
    self.badgeBGColor   = [UIColor redColor];
    self.badgeTextColor = [UIColor redColor];
    self.badgeFont      = [UIFont systemFontOfSize:1.0];
    self.badgePadding   = 6;
    self.badgeMinSize   = 2;
    self.badgeOriginX   = defaultOriginX-18;
    self.badgeOriginY   = 2;
