...
@interface SYPhotoViewController () <UIScrollViewDelegate, UIGestureRecognizerDelegate,UIActionSheetDelegate>
{
    UIImage *saveImage;
}
...
- (void)handleLongPressGestureRecognizer:(UILongPressGestureRecognizer *)longPressGestureRecognizer {
    if (longPressGestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        UIImageView *imageV=(UIImageView *)longPressGestureRecognizer.view;
        saveImage=imageV.image;
...
{
    if (buttonIndex==0) {//保存到手机相册
        NSLog(@"===========%ld=========saveImage=======%@",buttonIndex,saveImage);
...
#pragma mark - Private method（包括各种手势）
